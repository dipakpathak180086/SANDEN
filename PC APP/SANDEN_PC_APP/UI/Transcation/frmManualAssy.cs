...
{
    public partial class frmManualAssy : Form
    {

        #region Variables

        private BL_MANUAL_ASSY _blObj = null;
        private PL_MANUAL_ASSY _plObj = null;
        ...
        public frmManualAssy()
        {
            try
            {
                InitializeComponent();
                _blObj = new BL_MANUAL_ASSY();
        ...
        private void GetMachineStatus()
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    GlobalVariable.BindCombo(cbFinalStatus, dt);
                    cbFinalStatus.SelectedIndex = 0;
                }
                else
                {
        ...
        private bool ValidateSationChildPart()
        {
            ...
            {
                Common common = new Common();
                DataTable dt = common.ValidateStationWithChildPart(cbFinalStatus.Text.Trim(),txtScanBarcode.Text.Trim());
                if (dt.Rows.Count > 0)
                {
        ...
        private void txtScanBarcode_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                ...
                {
                    if (cbFinalStatus.SelectedIndex <= 0)
                    {
                        lblMsg.BackColor = Color.Red;
                        lblMsg.ForeColor = Color.Yellow;
                        lblMsg.Text = "Select Final Status!!!";
                        return;
                    ...
                    _plObj = new PL_MANUAL_ASSY();
                    _blObj = new BL_MANUAL_ASSY();
                    _plObj.DbType = "P2";
                    _plObj.Station_No = "03";
                    _plObj.MachineStatus = cbFinalStatus.SelectedValue.ToString().Trim();
                    ...
                    {
                        if (dataTable.Rows[0][0].ToString().Split('~')[0] == "1")
                        {
                            GlobalVariable.mStoCustomFunction.setMessageBox(GlobalVariable.mSatoApps, $"{txtScanBarcode.Text.Trim()} Saved  Successfully!!", 1);
